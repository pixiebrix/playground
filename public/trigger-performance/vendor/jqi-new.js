(() => {
  ...
  // vendor/new/jQueryInitialize.ts
  init_jquery_shim();
  ...
  // vendor/new/domUtils.ts
  init_jquery_shim();
  function $safeFind(selector, parent = document) {
    return $(parent).find(selector);
  }

  // vendor/new/routeChange.ts
  init_jquery_shim();

  // node_modules/simple-event-target/distribution/index.js
  init_jquery_shim();
  var eventType = "batik";
  var _target, _weakListeners, _SimpleEventTarget_instances, getNativeListener_fn;
  var SimpleEventTarget = class {
    constructor() {
      __privateAdd(this, _SimpleEventTarget_instances);
      __privateAdd(this, _target, new EventTarget());
      __privateAdd(this, _weakListeners, /* @__PURE__ */ new WeakMap());
    }
    /**
     * @deprecated Only for advanced use cases. Using this will break the type safety.
     */
    get raw() {
      return { eventType, eventTarget: __privateGet(this, _target) };
    }
    subscribe(callback, options) {
      __privateGet(this, _target).addEventListener(eventType, __privateMethod(this, _SimpleEventTarget_instances, getNativeListener_fn).call(this, callback), options);
    }
    unsubscribe(callback) {
      __privateGet(this, _target).removeEventListener(eventType, __privateMethod(this, _SimpleEventTarget_instances, getNativeListener_fn).call(this, callback));
    }
    emit(detail) {
      __privateGet(this, _target).dispatchEvent(new CustomEvent(eventType, { detail }));
    }
  };
  _target = new WeakMap();
  _weakListeners = new WeakMap();
  _SimpleEventTarget_instances = new WeakSet();
  ...
  getNativeListener_fn = function(callback) {
    if (__privateGet(this, _weakListeners).has(callback)) {
      return __privateGet(this, _weakListeners).get(callback);
    }
    const native = ((event) => {
      callback(event.detail);
    });
    __privateGet(this, _weakListeners).set(callback, native);
    return native;
  };

  // vendor/new/routeChange.ts
  var routeChange = new SimpleEventTarget();
  var installController = null;
  var lastUrl = "";
  function emit() {
  ...
  function subscribeRouteChange(listener, options) {
    install();
    routeChange.subscribe(listener, options);
  }

  // vendor/new/selectorTokenIndex.ts
  init_jquery_shim();
  ...
  var targetIndexes = /* @__PURE__ */ new Map();
  function getOrCreateIndex(target) {
    let index = targetIndexes.get(target);
    if (!index) {
      index = {
        idIndex: /* @__PURE__ */ new Map(),
        classIndex: /* @__PURE__ */ new Map(),
        complexFallback: /* @__PURE__ */ new Set()
      };
      targetIndexes.set(target, index);
    }
    return index;
  }
  function addToBucket(bucket, token, subscriber) {
  ...
  function removeFromBucket(bucket, token, subscriber) {
  ...
  function registerSelector(selector, subscriber, target) {
  ...
  function unregisterSelector(selector, subscriber, target) {
    ...
    if (index.idIndex.size === 0 && index.classIndex.size === 0 && index.complexFallback.size === 0) {
      targetIndexes.delete(target);
    }
  }
  function collectTokenTriggeredSubscribers(node, target, out) {
    const index = targetIndexes.get(target);
    if (!index) {
      return;
    }
  ...
  var EMPTY_FALLBACK = /* @__PURE__ */ new Set();
  function getComplexFallback(target) {
    return targetIndexes.get(target)?.complexFallback ?? EMPTY_FALLBACK;
  }

  // vendor/new/sharedMutationObserver.ts
  init_jquery_shim();

  // node_modules/abort-utils/dist/index.js
  init_jquery_shim();

  // node_modules/abort-utils/dist/on-abort.js
  init_jquery_shim();
  var createListener = (handle) => function() {
  ...
  function onAbort(signal, ...handles) {
    ...
    const targetSignal = inputSignal.aborted ? preAbortedHelper.signal : inputSignal;
  ...
  // node_modules/abort-utils/dist/merge-signals.js
  init_jquery_shim();

  // node_modules/abort-utils/dist/promise-from-signal.js
  init_jquery_shim();

  // node_modules/abort-utils/dist/link-controllers.js
  init_jquery_shim();

  // node_modules/abort-utils/dist/reusable-abort-controller.js
  init_jquery_shim();

  // node_modules/abort-utils/dist/signal-from-event.js
  init_jquery_shim();

  // node_modules/abort-utils/dist/signal-from-promise.js
  init_jquery_shim();

  // node_modules/abort-utils/dist/abort-all-and-clear.js
  init_jquery_shim();

  // node_modules/abort-utils/dist/promise-race-with-signal.js
  init_jquery_shim();

  // vendor/new/sharedMutationObserver.ts
  var routers = /* @__PURE__ */ new Map();
  function computeAttributeMode(subscribers) {
    let combined = "none";
  ...
  function collectTriggered(router, records) {
  ...
  function dispatch(router, records) {
    const snapshot = [...router.subscribers.values()];
    const triggered = router.handledCount > 0 ? collectTriggered(router, records) : null;
    for (const subscriber of snapshot) {
      if (triggered != null && subscriber.handle != null && !triggered.has(subscriber.handle)) {
        continue;
      }
      try {
  ...
  function subscribeMutations(callback, options) {
    if (options.signal.aborted) {
      return;
    }
    const router = getOrCreateRouter(options.target);
    const subscriberKey = /* @__PURE__ */ Symbol("sharedMutationObserver subscriber");
  ...
  // vendor/new/jQueryInitialize.ts
  var ENABLE_PERF_MARKS = false;
  var IGNORE_TAGS = /* @__PURE__ */ new Set(["STYLE", "BR"]);
  var BURST_FLUSH_THRESHOLD = 512;
  var THROTTLE_MS = 100;
  var ignoreTagsEnabled = true;
  var combinators = /* @__PURE__ */ new Set([" ", ">", "+", "~"]);
  var fraternisers = /* @__PURE__ */ new Set(["+", "~"]);
  var complexTypes = /* @__PURE__ */ new Set(["ATTR", "PSEUDO", "ID", "CLASS"]);
  var MutationSelectorObserver = class {
    constructor(selector, callback, options) {
      __publicField(this, "selector");
      __publicField(this, "callback");
      __publicField(this, "options");
      __publicField(this, "isCombinatorial", false);
      __publicField(this, "isFraternal", false);
      ...
      __publicField(this, "hasAttrToken", false);
      ...
      __publicField(this, "targetsIgnoredTag", false);
    ...
    try {
      ...
      for (const clause of tokens) {
        for (const part of clause) {
          ...
          if (part.type === "TAG" && typeof part.value === "string" && IGNORE_TAGS.has(part.value.toUpperCase())) {
            msobserver.targetsIgnoredTag = true;
          }
        }
      }
  ...
  function initialize(selector, callback, options) {
    ...
    const throttledCheckTarget = throttle_default(doDirectSweep, THROTTLE_MS, {
      leading: false,
      trailing: true
    });
    doDirectSweep();
        ...
        for (const mutation of mutations) {
          ...
          if (mutation.type === "childList") {
            for (const addedNode of mutation.addedNodes) {
              ...
              if (ignoreTagsEnabled && !msobserver.targetsIgnoredTag && IGNORE_TAGS.has(addedNode.nodeName)) {
                continue;
              }
              addedCount++;
              ...
              const scope = msobserver.isFraternal ? addedNode.parentElement : addedNode;
              if (scope != null) {
                matches.push(
                  ...$safeFind(
                    msobserver.selector,
    ...
    const attributeMode = msobserver.hasAttrToken ? "full" : msobserver.isComplex ? "id-class" : "none";
    const subscriberHandle = parseSelector(selector).kind === "complex" ? void 0 : /* @__PURE__ */ Symbol(`jQueryInitialize "${selector}"`);
    if (subscriberHandle !== void 0) {
    ...
    let routeSweepHandle = null;
    const cancelRouteSweep = () => {
  ...
  // entry-new.ts
  var g = window.PBXVendor || (window.PBXVendor = {});
